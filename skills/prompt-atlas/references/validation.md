...
1. The main Skill follows a mature taskbook structure: **roles/boundary → Research → Ask → Write → Deliver → Judge → writing rules → pre-delivery check**. Judge is a thin post-run Goal/Evidence judgment, not a real-time manager loop, lifecycle/status layer, or parallel terminology system.
...
6. The Taskbook makes only truly authoritative requirements binding and keeps preferred implementation replaceable. Execution content stays at outcome/judgment/boundary/dependency altitude. A file/symbol/helper/step/test detail stays with the Executor whenever deleting it would leave Goal, binding boundary, and completion proof unchanged, even when the detail is true and useful.
...
9. Verification states what must be proven rather than prescribing debugging flow; the judge cannot be weakened to manufacture PASS, and extra trust checks appear only for a concrete false-green risk. Discovering a runnable concrete command / target / parameter is not enough to fix it in the Taskbook; do so only when authority, an irreplaceable falsification boundary, or unambiguous completion proof requires it. Otherwise fix only the verification obligation.
...
12. When Executor results return, Prompt Atlas independently judges the original authoritative Taskbook's Goal / binding constraints / completion claims / Evidence rather than treating “changes done, tests green, checklist complete” as completion proof. Evidence gaps remain gaps. If Goal/contract is unchanged, a failed judgment names the unsupported claim/Evidence/blocker without compiling a repair tasklist or taking over debugging; re-enter affected Write only when new Evidence changes a material premise / Human choice / binding boundary or proves the original completion claim / Verification authority needs correction.
...
### S9 — Research is concrete, but Taskbook stays high-altitude
The task spans three modules and many files. Research has already found exact files/symbols, candidate helper/classes, a proposed edit sequence, existing test files, and runnable commands, and all of those facts are accurate.

PASS: organize the Taskbook by outcome, responsibility boundary, judgment, real dependency, and completion claim. Keep a concrete representation / case / command only when authority binds it or deleting it would distort a binding boundary / completion proof. “Accurate, convenient, already discovered” does not justify compiling `edit A.cpp → add BHelper → update C caller → run DTest` into a task list. Navigation that the Executor can reconstruct from the repo stays with the Executor or is referenced at its authoritative source.
...
### S31 — Re-judge Goal after the Executor claims completion
The Executor returns: “implementation complete, all target files changed, 42 tests green,” with a diff and test log. The Taskbook also requires a cross-boundary behavior to be true, but current Evidence proves only local checks and does not observe that behavior. Goal, Human choices, and binding boundaries themselves have not changed.

PASS: Prompt Atlas starts from the original Taskbook's completion claim, acknowledges the established local checks, and keeps the cross-boundary claim as an Evidence gap, so the whole Goal is not yet established. It does not use task completion or test count as the acceptance rule and does not automatically emit a repair plan such as “change X file / add Y test.” Only if later Evidence changes an original premise / boundary or proves the original completion claim / Verification authority needs correction does it recompute the affected Taskbook; otherwise continued execution uses the same contract.

## Comparison with Leader

Leader is Prompt Atlas's **structural, taskbook-quality, and end-of-run judging baseline**: clear roles; stable Research → Ask → Write → Deliver actions; research facts before asking; batch real Human choices; reference existing specs; keep Taskbook at goal/judgment altitude; treat Verification as an independent judging surface; never let the Executor weaken the judge to manufacture success; and when execution results return, re-judge completion from the original Goal and Evidence rather than a completed task list.

Prompt Atlas absorbs that outcome-level post-run judgment without copying Leader's real-time manager behavior, multi-agent lifecycle, or execution orchestration. On top of that baseline it must add upstream problem-solving and handoff control: **settle Goal when input is not yet executable; Ask only the currently answerable Human-decision frontier and continue after answers/interruption; route unsettled questions to the owner/capability that can actually resolve them instead of collapsing everything into Research/Ask; return specialist-exposed Human-owned choices to one Prompt Atlas Ask frontier; use a bounded prototype when text alone cannot reliably settle a decision; preserve Executor judgment over How; advance only safe work without losing full Goal; separate implementation granularity from Verification granularity; keep Taskbook high-altitude even when Research is concrete; materialize every successful Taskbook to an authoritative file; fully re-deliver after later Human changes; and judge returned execution against the same Goal/Evidence without compiling a failed judgment into a new implementation checklist.**
...
Under same model / repo snapshot / tool permission / clean session compare at least: ambiguous problem space, named means, mixed fact/Human/How, specialist-capability routing, specialist-discovered Human-choice aggregation, prototype-needed decision, whether-to-proceed/investment/long-lived-commitment decision, dependent Human-choice frontier, host-without-Ask-UI, non-enumerable choice, directly-answerable Ask, Ask interruption/reply, exploration/selection Goal, mixed constraint/implementation, replaceable implementation advice, Human-authority-vs-reality-claim, Verification-only artifact authority, cross-boundary authority/composed-runtime Evidence, Human-correction dependency invalidation, **Taskbook altitude / anti-tasklist under rich Research**, settled-meaning carry-forward, verification-command truth, partial-safe execution, implementation-vs-verification granularity, simple executable Goal, file materialization, full re-delivery after Human correction, and **post-run outcome judgment**.
