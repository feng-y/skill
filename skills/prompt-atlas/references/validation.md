...
4. Ask is a gate independent of host interaction mode and delivers the current frontier as one directly replyable decision packet of at most 5 choices. Minimum sufficient Evidence, choice impact, 2–4 credible mutually exclusive same-level options with stable labels for each enumerable choice, main consequences, a clear recommendation, and compact reply syntax are all required; a choice that cannot be enumerated reliably uses a bounded free response rather than invented options. Downstream choices that depend on an open Human decision are not asked early, and a route-changing Human choice cannot be bypassed with a provisional Taskbook or implicit default. After a partial answer, new constraint, or corrected premise, Prompt Atlas replaces the premise and recomputes its dependency cone through Goal / Ask / Taskbook / Verification; unrelated closed choices are not re-Asked, and Write/Deliver resumes once remaining choices stabilize.
5. When cross-boundary authority / provenance / lifecycle or a composed runtime decision can affect Goal, Ask, a binding constraint, Verification, or the first safe material work, Prompt Atlas obtains Evidence from the real workflow and preserves every route-changing input that jointly determines behavior; a current narrative or artifact presence does not replace authority. Unresolved route-changing facts remain explicit Unknowns, and Research stops only when more context changes How. Authoritative specs are referenced directly; current workspace is reality, not a reason to require clean state or treat an existing diff as correctness proof.
...
Under same model / repo snapshot / tool permission / clean session compare at least: ambiguous problem space, named means, mixed fact/Human/How, specialist-capability routing, specialist-discovered Human-choice aggregation, prototype-needed decision, whether-to-proceed/investment/long-lived-commitment decision, dependent Human-choice frontier, decision-packet shape, Ask interruption/reply, exploration/selection Goal, mixed constraint/implementation, replaceable implementation advice, cross-boundary actor/lifecycle/provenance Evidence, Human-correction dependency invalidation, Taskbook altitude, settled-meaning carry-forward, verification-command truth, partial-safe execution, implementation-vs-verification granularity, simple executable Goal, file materialization, and full re-delivery after Human correction.
