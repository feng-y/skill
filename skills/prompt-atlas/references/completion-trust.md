...
- **Current Context** — existing gates, their strength or false-green risk, relevant baselines, available independent verifier, and whether the relevant facts are verified, inferred, or still unmeasured.
...
- **Success / Stop** — evidence required for completion, the rule that insufficient or compromised proof cannot complete, and whether the current execution closure is `complete`, `ready for independent acceptance`, or `blocker`.
