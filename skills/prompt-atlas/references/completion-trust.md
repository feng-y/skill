...
5. **Independent challenge** — executor self-attestation is sufficient when direct proof reaches the target, the gate is failure-sensitive where needed, plausible shortcuts are frozen, and residual risk is controlled. Require a fresh-context verifier or governing external acceptance when a repo/user rule requires it or material residual risk remains. Treat a behavior-preserving change whose preserved behavior is outside the governing acceptance path, or another result whose decisive gate is designed by its executor, as requiring this challenge; other examples include a critical user path or architecture work whose claimed structural gain lacks strong direct evidence. Name the available accepting boundary in the artifact. Until that boundary passes, executor evidence changes the state only to `ready for independent acceptance`, not global `MET`.
