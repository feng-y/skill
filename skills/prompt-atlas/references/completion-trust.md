...
Use a fresh or independent Acceptor (a judge that did not do the work) when Executor self-attestation remains gameable or incomplete. Bind or preserve the accepting boundary before execution so a new conversation can judge the result. Keep hidden cases and private proof outside the Executor-visible taskbook. If no accepting environment exists, the highest honest result is `ready for independent acceptance`, not `PASS`.
