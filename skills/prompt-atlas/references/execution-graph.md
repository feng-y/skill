...
When an unresolved area blocks progress, apply the Unknown closure operations defined in [SKILL.md](../SKILL.md). Compile only the smallest next operation that can truthfully close or sharpen the gap. Operations may chain, but do not freeze the full closure path in advance or turn them into fixed Task types, workflow stages, or substitutes for authority judgment.
