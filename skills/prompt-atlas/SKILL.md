...
Keep these semantics distinct during reasoning. In Artifact mode, do not collapse a stable result into free-form prose merely for human editability. Compile it into the fixed agent-facing contract below. Structure is part of contract correctness: current frontier-model guidance benefits from explicit goals, relevant context, constraints, success/evidence requirements, output expectations, and unambiguous handoff rather than requiring the next agent to infer those roles from prose.
