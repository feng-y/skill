...
Prompt Atlas is done when the next consumer can continue without re-deriving what the user means, reopening superseded intent, re-framing an already-visible user decision, or recreating an evidence-backed recommendation Atlas should already have surfaced. Downstream work may enrich implementation knowledge and current evidence, but must not reopen settled user intent without new user authority, demote a hard boundary into a suggestion, or turn an unresolved proof gap into completion; contradictory territory evidence may reopen assumptions, feasibility, or route, not user intent.
