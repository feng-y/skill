...
In Artifact mode, return one concise Intent Contract, requested carrier, or smallest necessary decision surface. Stable artifacts end with a brief Handoff Hint choosing Direct Execute or Wayfinder; normally keep the hint to 1–3 sentences. For Chinese output, target roughly 100–200 characters and keep routine artifacts within 1,000 characters. Treat these as normal output budgets, not completion gates: exceed them when fitting the budget would materially reduce goal attainment or stability. In Embedded mode, return the compiled artifact or decision surface to the caller. In both modes, delivery ends the Prompt Atlas invocation.