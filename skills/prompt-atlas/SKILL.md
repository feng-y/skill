...
Autonomous handoff is delivered through a materialized Taskbook: write the current compiled definition to an OS/runtime temporary Markdown file outside the current repo/workspace, without hard-coding a fixed path or filename. That file is the fresh Executor's authoritative contract; do not emit `Status: Executable` until the write succeeds. Surface the actual path immediately after Status. If the same task later receives a material correction, update the same file while it remains accessible; if it is no longer accessible, materialize a replacement and explicitly mark it as superseding the earlier handoff. If materialization cannot be completed, return accurate `Blocked` with a resume condition rather than treating chat prose as a delivered file.
...
- `Status: Blocked` — a factual/environmental blocker leaves no safe continuation; state the resume condition. An autonomous handoff that cannot be materialized is still undelivered.
- `Status: Executable` — the ordinary text task definition is complete, or the autonomous Taskbook has been successfully materialized; the latter must surface its authoritative path immediately.