...
An autonomous handoff is complete only after its authoritative Taskbook is materially delivered; do not emit `Status: Executable` before delivery. A material correction to the same handoff must continue until the updated authoritative Taskbook is delivered. If delivery cannot be completed now, return accurate `Blocked` with a resume condition rather than substituting chat prose or a promise to write later.
