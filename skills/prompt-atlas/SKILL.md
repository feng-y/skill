...
Research closes only Compile blockers before Handoff; it does not try to understand the complete execution reality in advance. Load only context whose absence now could materially change Goal/authority, initial safe Execution, binding Verification, or Evidence judgment. **Once Goal/authority is stable, current Evidence is enough to compile at least one safe material Task or truly necessary Task 0, and Verification authority/trigger is clear enough for the taskbook to be handed off safely, stop Research and enter Compile/Handoff.**

Check only the workspace, governing specs/tests, critical commands, baselines, dependencies, and repo verification authority that Handoff correctness actually depends on. A new observation that reveals another consumer, dependency, historical clue, or implementation question is not by itself a reason to continue Research; unless it becomes a Compile blocker, the Executor obtains Evidence only when it actually affects current ready work. Task 0 is only for execution-time facts that must be closed before the first material action; it is not a holding area for unresolved execution detail. Treat documentation and command names as claims until evidenced. Important conclusions need a source pointer or reproducible observation; a summary is not proof.
...
- **Execution / Graph** compiles the best-known complete Tasks / relations that current Evidence already establishes. Preserve any still-valid Human-confirmed execution strategy, scope boundary, or must-preserve constraint rather than replacing it with a model-invented decomposition. Prefer the smallest ready frontier that directly advances the Goal; do not materialize execution Unknown as prerequisite Tasks while they do not block the current frontier. **The ready frontier controls what can execute now; it must not redefine the Human-owned Goal as a layer, phase, or subgoal.** When current Evidence already supports a bounded path that can keep advancing, compile it directly; materialize only affected contingent work when new Evidence actually blocks or changes current work. Keep simple work linear and read [execution-graph.md](references/execution-graph.md) only when a linear list would hide a real relationship. Delay only work whose existence, scope, or relationship remains materially contingent on future Evidence. **Best-known complete means complete enough for the work structure already established by current Evidence; it does not require complete repo/dependency/reachability knowledge before execution starts. The Graph stop boundary comes from Goal/confirmed boundaries, and adjacent residual discovered during execution does not automatically expand scope;**
