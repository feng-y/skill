...
- **Artifact mode** — return the completed Intent Contract or requested carrier, optionally with one concise next-capability hint when useful.
- **Embedded mode** — establish the contract internally, then return the contract and control to the calling workflow; optionally name the next capability, but do not perform downstream work as part of Intent Take.
