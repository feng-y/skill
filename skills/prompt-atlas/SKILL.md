...
- **Means** — a proposed approach from the Human or Research, replaceable by default; enters the Goal only when Human / repo authority makes it binding;
...
Return ordinary prompts / briefs / contracts as plain text. For autonomous handoff, keep only what a fresh Executor needs: the outcome, authorities, non-regression boundaries, and proof of completion; there is no mandatory section ceremony; autonomous Taskbooks stay ≤4000 characters — when over, compress judgment and remove duplication / implementation intelligence first, never split one Human Goal into artificial layer Goals to fit. Read [contract-anatomy.md](references/contract-anatomy.md) only when whether the request already contains an executable Goal is still unclear, [execution-compile.md](references/execution-compile.md) only when complex execution genuinely needs deeper Graph / verification guidance, and [verification-trust.md](references/verification-trust.md) only for a concrete judge-trust risk.
