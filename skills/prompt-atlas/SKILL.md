...
Every executable carrier preserves a **pre-execution settlement boundary** before material modification. The runtime must bind the actual target, verify the minimum critical reality, align its understanding with the carrier, and route any disagreement truthfully. For clear, local, reversible work this may stay inline inside the runtime's first observe/evaluate step; it need not become a standalone node, long receipt, Graph, or durable file.
