...
Routing assigns consequence and authority; it does not by itself close the Unknown. Use the smallest closure operation that makes the next judgment possible: `Research` reads existing reality; `Prototype` creates a cheap, rough, concrete artifact that makes an abstract question judgeable; `Grilling` obtains a decision from the Human who owns it after enough currently available evidence has been gathered to support the judgment; closure `Task` means a necessary real-world enabling action when observation or discussion cannot proceed, used to unblock judgment rather than deliver the Goal itself. These operations may chain, but they are not fixed Task types or another workflow. Any new fact, artifact, or option they produce still follows the original Unknown's consequence and authority route; closure operations never replace Human authority.
