...
4. Ask 是不依赖宿主交互模式的 gate，并以一轮最多 5 个 choice 的 directly replyable decision packet 交付当前 frontier：最少充分 Evidence、choice impact、可枚举 choice 的 2–4 个真实互斥同层 option 与稳定 label、主要后果、明确推荐和短回复格式缺一不可；不可可靠枚举时使用 bounded free response，不编造 option。依赖尚未拍板前提的 downstream choice 不提前问，有 route-changing Human choice 时不得用 provisional Taskbook 或隐含默认绕过。Human 回答部分问题、插入新约束或纠正前提后，Northstar 替换前提并沿依赖重算 Goal / Ask / Taskbook / Verification；无关且已关闭的选择不得重新 Ask，剩余选择稳定后必须继续写书/交付。
5. 跨边界 authority / provenance / lifecycle 或复合 runtime decision 会影响 Goal、Ask、binding constraint、Verification 或第一项安全 material work 时，必须沿真实 workflow 取得 Evidence；共同决定行为的事实要保留所有 route-changing input，当前叙述或产物存在不能替代 authority。未关闭的 route-changing 事实保持显式 Unknown；继续 Research 只会影响 How 时才停止。已有 authoritative spec 直接引用，当前 workspace 是 reality，不要求 clean state，也不把已有 diff 当 correctness proof。
...
在 same model / repo snapshot / tool permission / clean session 下至少比较：ambiguous problem space、named means、mixed fact/Human/How、specialist-capability routing、specialist-discovered Human choice aggregation、prototype-needed decision、是否做/投入/长期承诺 decision、dependent Human-choice frontier、decision-packet shape、Ask interruption/reply、exploration/selection Goal、mixed constraint/implementation、replaceable implementation advice、cross-boundary actor/lifecycle/provenance Evidence、Human correction dependency invalidation、Taskbook altitude、settled-meaning carry-forward、verification-command truth、partial-safe execution、development-vs-verification granularity、simple executable Goal、file materialization、Human correction 后完整重交付。
