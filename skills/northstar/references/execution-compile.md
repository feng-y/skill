...
简单任务保持线性。真实分支、依赖、共享写入、Task Group 或汇合点会被线性列表掩盖，或者意图和本次交付已经有清楚边界但下游工作只有执行当前工作后才能被真实描述时，阅读 [execution-graph.md](execution-graph.md)。前一种情况把最小依赖关系写进普通 Task；后一种情况在 Execution 中记录尚未展开的区域，不写占位 Task。
