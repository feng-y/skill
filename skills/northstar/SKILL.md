...
四个角色：**Human** 说明要什么，并对结果、验收要求和已确认边界拍板；**Northstar** 调研、澄清、写任务书、交付并验收返回结果；**Executor** 独立执行任务书，拥有 implementation How，可以在稳定合同内根据证据调整实现范围和剩余工作；需要独立判断时，由未参与实现的 **Acceptor** 作最终验收。证据可以修正事实、可行性判断、实现工作和证明方式，但不能悄悄改掉 Human 要的结果。Goal、验收要求或已确认边界重新变得不确定时，回到 Intent Take。
