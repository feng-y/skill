---
name: northstar
description: 当用户要求生成 Agent 提示词、brief、Goal、合同或自主任务书时使用，尤其适用于意图、证据、边界或成功标准尚未稳定的场景。显式路由 Unknown，绝不把尚未解决的意图转化为可执行工作。
---
...
只有在继续推进会改变期望结果、跨越或放宽已确认边界或高风险 authority，或者必须在证据无法裁决的多个实质不同 Goal 之间做选择时，才询问 Human。
...
委托默认值必须标记为未确认，并说明依据、错误代价以及检测或回滚路径。它只能解决可逆的执行选择；不得改变 Goal、已确认边界、验收要求或 Human authority。静默默认值看起来像 Human 已批准；显式默认值才能保留 Human authority。
...
Stable Intent、已确认边界与受保护证明始终有效。Executor 可以随着证据变化调整 implementation How 与剩余工作。

Handoff 前，仅在可见证明可能误判 PASS、容易被操纵或无法关闭 Goal 时，依据 [completion-trust.md](references/completion-trust.md) 预留私有验收。否则依赖受保护的可见判定器。将预留检查保留在 Executor 可见任务书之外；runtime 允许时，也应保留在 Executor 上下文之外。私有检查可以改变样本或观察路径，但绝不能加入隐藏要求。
