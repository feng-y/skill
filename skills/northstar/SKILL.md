...
- **Intent take**：接住 Human 的最新意图并结合必要 reality，恢复 Human 真正接受的 Goal、binding constraints、authority boundary，以及仍需 Human 决定的 material choices。只关闭会改变 executable contract 的 intent ambiguity，不追求完整需求穷举；点名的实现、architecture、tool 或 patch 默认只是 means，除非 Human / repo / upstream authority 让 representation 本身 binding。
- **Intent compile**：在 Intent 已足以约束执行后，先确定 fresh Executor 可直接使用的 **executable Taskbook contract**，再用 Graph 组织其中的 Execution。Executable 表示 Executor 不需要重新猜 Human intent、重新定义 material boundary 或重新发明 completion proof；Graph 只结构化 Execution，不定义 Intent，也不表示 Northstar 要提前决定 implementation How、verifier composition 或未来 contingent work。
...
- **Goal**：Human 最终会验收的结果。
- **Taskbook**：Intent compile 的 executable contract；至少承载 Goal / binding constraints / authoritative references、会改变判断的 material work / boundaries / real dependencies，以及 completion claims / Evidence obligations。Intent compile 先确定这个 contract，Graph 只负责表达其中 Execution 的 material work 与真实 dependency。
...
**1. Intent take（定准）。** 从 Human 最新且仍有效的表达开始，区分 accepted outcome、可替换 means、binding constraint 与真正需要 Human authority 的选择。Intent take 只关闭会改变 executable contract 的 ambiguity：如果不同答案不会改变 Goal、Human-owned choice、binding boundary、material work judgment 或 completion obligation，就不继续为了“理解完整”而展开。优先引用已有 tests/schema/ADR/Architecture Intent/验收脚本等 authority，不复制第二份 prose SOT。只调查会改变 Goal、Human-owned choice、binding boundary、material work 判断、completion obligation，或当前 material work 能否安全开始的 reality；Taskbook 判断或 binding rule 真正依赖的 reality claim 在交付前必须有足够 Evidence。当前叙述、artifact presence 或模型高置信方案本身都不构成 authority。当前 workspace 中仍有效的修改就是 reality，不要求 clean state，也不把已有 diff 当 correctness Evidence。
...
**2. Intent compile（编译）。** Intent 已足以约束执行后，先确定当前 executable contract，再组织其中的 Execution。Taskbook 必须让 fresh Executor 可以直接推进，而不是只得到 Goal 摘要或 task list；只保留 fresh Executor 不知道就可能判断错、越界、重新猜 Human intent 或无法证明完成的信息：
...
1. Intent take 是否只关闭会改变 executable contract 的 ambiguity，并恢复 Human 真正接受的 Goal、binding constraints、authority boundary 与仍需 Human 决定的 material choices，而没有把 means 当 intent 或追求完整需求穷举？
