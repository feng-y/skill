...
`Unknown` 是贯穿整个 Skill 的顶级控制。任何阶段出现新的 Unknown，都先按后果路由；Intent Take 只处理会影响 Goal、验收要求或已确认边界的 Unknown，其他 Unknown 分别进入调研、Task 0、Executor、替 Human 作出的可回退决定、分支暂停或 `Blocked`。
