...
路由只确定 Unknown 的后果和权限归属，不等于已经把它关闭。选择能让下一步判断成立的最小闭合操作：`Research` 读取已有现实；`Prototype` 构造一个低成本、粗糙但具体的产物，把抽象问题提高到足以判断的分辨率；证据已经足够后，`Grilling` 从真正拥有决定权的 Human 获得决定；闭合语义中的 `Task` 指只有完成必要现实操作才能解除判断阻塞的前置工作，而不是直接交付 Goal。这些操作可以串联，但不是固定 Task 类型，也不增加新的 workflow，更不能代替 Human 权限。
