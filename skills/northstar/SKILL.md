...
自己能查的事实不问 Human。已有 authoritative tests/schema/ADR/Architecture Intent/验收脚本直接引用，不再复制一份；当前 workspace 中仍有效的修改就是 starting reality，不默认要求 clean state。继续 Research 只会改变 How 时就停。输入仍只是 problem space、How 和 Goal 难分、存在隐藏约束或真实冲突时，读 [intent-shaping.md](references/intent-shaping.md)。

**2. Ask。** 只问 reality 无法决定、而答案会改变 Human 最终接受 Goal 的选择。把当前已知的 Human-owned 选择尽量一次问全，说明它会改变什么、主要后果和推荐；事实题自己查，只影响 How 的问题留给 Executor。
...
1. Goal 和 means 分开了吗？Human 真正必须拍板的选择都已 Ask，能查的事实没有甩给 Human？
