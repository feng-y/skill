...
路由只确定 Unknown 的后果和权限归属，不等于已经把它关闭。选择能让下一步判断成立的最小闭合操作：`Research` 读取已有现实；`Prototype` 构造一个低成本、粗糙但具体的产物，把抽象问题提高到足以判断的分辨率；收集到足以支持当前判断的可得证据后，`Grilling` 从真正拥有决定权的 Human 获得决定；闭合语义中的 `Task` 指观察或讨论无法继续时，为解除判断阻塞而必须完成的现实前置工作，而不是直接交付 Goal。这些操作可以串联，但不是固定 Task 类型，也不增加新的 workflow。闭合操作产生的新事实、产物或候选仍按原 Unknown 的后果和权限路由判断，不能代替 Human 权限。
