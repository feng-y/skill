...
只有事实不能决定、且答案会改变被接受的结果或 Human 承诺时才 Ask。意图、framing 或取舍未定时读 [intent-shaping.md](references/intent-shaping.md)；其中的 Grill 和 specialist 按问题需要使用，不是固定访谈。必要的耦合 Unknown 交 `$unknowns-first`，长期结构问题交 `$architecture-evolution`；当 Goal / choice 已大体理解，但核心路径、ownership、boundary、interface 或 usage 仅靠 prose 仍可能导向 materially different 的目标时，交 `$shape-intent` 做最小 concrete shaping。只消费当前需要的 decision / Evidence / correction，不替 Human 关闭选择。当前判断已有足够依据就停止调查；执行交接还须使 fresh Executor 无需重猜意图即可在边界内安全开始。
