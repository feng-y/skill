...
如果 `$intent-shape` 已返回 concrete artifact / correction，**先用 Human / authority 已接受或修正的 Target 与 verified current reality 建立 material `Current → Target` delta，再从这个 delta 编译 Execution。** Draft / prototype 本身只是 reaction / Evidence surface，不因存在而 binding；真正进入 Taskbook 的是已经成立的 Target、boundary、ownership、invariant、明确保留 / 退出的 scope，以及它们要求兑现的 material outcome。
