...
Intent 稳定，意味着这些事情已经清楚：唯一、内部一致且由 Human 决定的 Goal、为什么做、已确认边界、重要证据现状、怎样才算真正且可信地完成，以及最终交付什么。否则返回 `Status: Unresolved Intent`，写清当前理解和最小有效问题或探针。**意图未解决，绝不输出可执行工作。**
...
Stable Intent、已确认边界和不可削弱的证明要求仍然具有约束力。证据变化时，Executor 可以调整实现方式和剩余工作。
