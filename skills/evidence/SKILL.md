---
name: evidence
description: "Own engineering proof semantics: start from an authoritative completion/safety claim, choose the closest real-artifact observation and appropriate verification backend, collect or request proof, and judge proven / false / unproven without owning implementation or the verifier runtime."
---

# Evidence · 证明开发结果符合预期

Evidence 负责回答一个独立问题：**当前 engineering claim 到底被什么事实证明，现有证据是否已经足够。**

它不拥有 Intent，不定义长期 Architecture，不实现代码，也不拥有 test/build/replay/runtime harness。Northstar 定义 accepted outcome / Acceptance；Architecture Evolution 定义 adopted structural Target / completion semantics；Evidence 把这些 authoritative claims 转成 proof obligation，选择最直接的 observation surface 与可用 verifier/backend，消费结果后判断 `proven / false / unproven`。

在 DaVinci 中，Replay 是可执行的 verification system/backend，而不是这个 Skill 的 semantic owner。Evidence 可以调用或消费 Replay，就像消费 build、test、runtime probe、data readback、profile 或其他已有 harness 一样。

核心规则：

> **先固定 claim，再看 real artifact。Verifier 负责运行，Evidence 负责证明。**

## 什么时候调用

适合：

- 需要证明一个 feature / bug fix / migration / refactor 是否真的满足 Acceptance；
...
- Architecture Evolution 已采用 structural Target，需要证明 owner / dependency / authority exit 已真实落地；
- Human 明确要求“证明它符合预期”“给 Evidence”“独立验收”。

trivial/local change 若一个已有 authoritative focused test 已直接覆盖 claim，Evidence 可以接受该 proof，不为了流程完整强制独立 harness 或额外 Skill ceremony。

## Claim first

先读取 authoritative contract，只恢复当前 proof 需要的 claim：
...
## Verifier / backend 不是 semantic owner

Evidence 优先复用项目已有 verification system：

- DaVinci harness Replay；
- repo 的 tests / build / integration / equivalence harness；
- project-local verification skill / control harness；
- runtime probe、data query、profile、trace 或其他 direct observation。

Verifier 的职责是**执行并返回 observation/artifact**。Evidence 决定为什么运行它、输入身份是否正确、结果证明哪个 claim，以及 proof 是否充分。
...
如果这些事实未知，交 `$unknowns-first` 关闭 factual gap；Evidence 不用猜测的 baseline 判卷。

## Proof judgment

对当前 scope 内的 material claims只给三种状态：
...
`unproven` 不是 implementation defect；“没发现反例”也不是 proven。无法执行需要的 verifier 时，明确报告 `unproven` 与恢复条件。
...
Evidence 先给 verdict，再按 premise 路由：
...
- verifier/backend 本身坏或不可运行 → 报 backend blocker，不把 product 判成 false。
...
Evidence 验证**产品/工程 claim**。Skill/prompt/harness 语义变更本身是否改善 agent behavior，应该使用 blinded behavioral eval，而不是拿产品 Replay 代替。候选必须在 clean / sanitized session 中运行，judge 看真实产物与行为，而不是候选自报“我遵守了规则”。
