...
description: 当工程意图、相关目标或指定范围需要长期责任、边界、依赖与架构取舍判断时使用。依据 repo identity 与现实推导 AI-native Target，必要时收敛有 structural gain 和 real exit 的 Program，不接管需求 shaping 或实现。
---
...
输入可以沿用 Human 表达、issue、specification 或 Goal，不要求独立 Goal 文件或先调用 Northstar；需要的是足以判断当前结构问题的结果、约束和范围。AE 在这些边界内给出明确技术判断，Human 保有业务优先级、投入、兼容与风险承诺。
...
**1. 看 Goal、变化压力与现实。** 从当前有效意图恢复会改变架构判断的 repo identity / architecture intent，并核对责任、依赖、权威、生命周期、公开边界和真实摩擦。AE 不重新做需求 shaping。未决 Human choice 下，若一个显式假设的结构问题可独立回答，先给条件性结论、后果与 Evidence，不把它当已批准 Target / Program；其余需要 Human commitment 的判断返回 decision surface，不自行猜。
...
剩余 material fork 若不是事实问题而是 Human-owned 的业务、兼容、风险、投入或 ownership 取舍，停止对该 fork 的 research，返回 Evidence、best-known recommendation、真实选项和 decision surface；不把条件性技术分析当作关闭承诺，也不冻结无关且已有依据的判断。
...
**4. 交付。** 局部问题只给局部判断；缺关键事实或人的决定时，说明缺口与继续条件。上游只问条件性结构后果时，返回该问题的结论、假设和依据，不生成完整 Program。收敛的设计完整给出：
...
- 当前值得推进时的 Program structural moves、real exit 与 migration boundary；
...
决定已成立但下游需要复用其依据时，保留真正决定选择的 Evidence / binding constraint，以及必要的 material alternative / reopen condition；没有真实分歧不凑备选，不额外引入强制 ADR 或 decision ledger。依据可解释不等于 structural outcome 已证明，仍按上述 claims 验证。交给 Northstar / Executor 时引用原 authority，不重新编造意图或复制架构 SOT。

把收敛的设计 / Program 同一正文写入仓库或工作区外的 Markdown 交接文件；若获授权，稳定结论更新原 authoritative architecture source，否则交接文件不成为 repo SOT。Human decision / 实质修正后保留仍有效 Evidence / Target，只重算受影响判断；前提失效才重查对应部分，并完整重交付。写入失败就是阻塞。不要输出 `ready`、`completed`、`executable` 或 `status` 状态词。
...
只有用户、仓库或上游权威来源，以及已验证的现实，才能绑定长期承诺或当前事实。架构文档是 Evidence / 稳定结论的持久化来源，不是不可挑战的 oracle。除非某种表示已被权威固定，AE 只规定架构结果和 structural done condition，不提前规定类、接口、文件、配置、调用形态、合并请求、任务拆分或测试工具。AE 在当前委托范围内收敛；若被上游 shaping capability 调用，只返回 Evidence、方向和 Human decision surface，不接管提问。新 Evidence 仅揭示实现违反既有结构约定时不重做战略设计；推翻结构前提才重开受影响 Target，改变 accepted outcome 或 Human commitment 才回意图 owner。
