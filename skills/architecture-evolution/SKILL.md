...
- **Layering + cohesion。** 保持少量稳定单向依赖，common/policy 不知道 specific；模块围绕主要 capability 闭合 knowledge/state/lifecycle，caller 不重组私有事实。两个结构都成立时选概念更少、public surface 更小的。
...
3. 每个 Improvement 是否完成即 structural gain + real exit，且没有 research/setup/future/implementation step？
