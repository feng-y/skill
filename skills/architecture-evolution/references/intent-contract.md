...
```markdown
## Acceptance derivation / current snapshot evidence
- Acceptance derivation: <实现验收时如何从最终 change surface 和届时有效 binding/config/runtime reality 推导 affected scope；仅动态 scope 时需要>
...
- consumer reassembly 是 cross-cutting signal，不是第五个架构方向；减少 consumer knowledge 不自动意味着按当前 consumer 形状创建 public view/interface；
- Stable Abstraction 的 variation 必须由 semantic / invariant evidence 支撑；provider 名称、当前 execution shape 或 class hierarchy 不自动成为长期 taxonomy；
...
- Challenge 必须检查 false unification、历史差异固化、speculative abstraction、复杂度转移、consumer reassembly、owner-scope expansion、variation-axis lock-in 和退出真实性；
- reasoning distinction 或 consumer slice 不自动成为 type、provider、adapter、view、layer 或 public seam；
- Brooks 是下游架构设计逐步吸收的约束；intent 只携带相关约束和必要 proof expectation，不输出全量风险表、Severity、PASS/RETRY 或 Health Score；
- Success evidence 先描述必须证明的 properties；build/test/replay/dependency check 等 evidence provider 由后续阶段基于 repo verification authority 和最终 change surface 选择，除非某入口本身是稳定受保护判卷标准或必须点名才能消除歧义；
- affected scope 只有动态变化时才要求实现验收时重新推导；当前具体对象只作为 current snapshot evidence；
