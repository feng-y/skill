...
## Success evidence
- Stable acceptance rule: <实现完成时必须成立的 invariant / proof；affected scope 会动态变化时再写届时的推导方法>
...
如果当前具体样本有助于说明 grounding，可以在 `Success evidence` 后追加：
...
- Success evidence 必须以稳定 acceptance rule 为主；只有 affected scope 会动态变化时才要求实现验收时重新推导，当前具体对象只作为 current snapshot evidence；
