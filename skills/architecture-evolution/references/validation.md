...
8. Research bounded；恢复 repo identity 不等于全 repo inventory；Target 与当前 Program 分离，gap 不自动成为 obligation，Program 只保留现在值得推进且 independently improving 的 changes；
...
12. delivery 交当前 Strategic Design、必要的 Target 变化与 Program，且不是 lifecycle state；material update 后必须完整重交付；
