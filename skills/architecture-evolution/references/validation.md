...
- **V6 Durable leverage**：一次性 migration 当前确实跨多个 owner / authority / verification surface；候选 Target 也能把这一次 change 压缩到更局部并有 real exit，但 migration 完成后没有已证实 recurring change、ongoing pressure 或 binding future requirement 会继续经过该 boundary → 不得仅凭当前一次 propagation compression 晋级为优先 Architecture Improvement；优先 local/implementation-level 收口。若未来变化由 roadmap / authority 明确绑定，则按 V4 处理，不要求历史重复。
...
2. Target / Improvements 必须从有 Evidence 的真实 change pressure 暴露的结构约束出发，并压缩一类已证实变化的传播范围；这种 leverage 必须作用于已证实持续/重复的 change pressure 或 binding future requirement，而不能只优化一项完成后即消失的一次性 change；已批准 roadmap / binding future requirement 不要求历史重复，纯猜测不行；结构正确但不改变 change propagation 的 cleanup 不够；
