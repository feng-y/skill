---
name: unknowns-first
description: "Close factual map-versus-territory unknowns with the smallest useful probe or source alignment. Route intent, concrete-shape, architecture, and proof-sufficiency questions to their semantic owners instead of solving them here."
---
...
Unknowns First 不拥有 Intent，不做 concrete-shape design，不决定 Target Architecture，不定义 completion contract，也不判断一个 engineering claim 的 proof 是否充分。
...
- **一个 accepted completion/safety claim 应由什么证明、现有 proof 是否足够？** → `$evidence`。
...
`$evidence` 消费这些 factual Evidence，对 authoritative engineering claim 做 proof obligation 与 `proven / false / unproven` judgment。若 Evidence 只是缺一个事实，可以调用 Unknowns First；事实关闭后立即返回 Evidence。

Unknowns First 不因为“某个事实已成立”就推导 whole-outcome PASS。例如确认 production 仍有 legacy path 是一个 fact；它是否反证当前 replacement claim 由 Evidence 判断。
...
- 把 verifier output 当成 whole-outcome proof judgment，而不是返回 Evidence。
