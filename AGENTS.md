...
`prototype` 仅作为迁移期 compatibility alias；prototype/mock/minimal implementation 仍可以是 Beacon 的 implementation technique，但不再拥有独立 semantic owner 身份。
...
- **Beacon artifact**：bounded reaction / inspection surface，通常可丢弃；durable correction / Evidence 返回原 caller，由 caller 决定是否需要 fold back。Prototype 只是其中一种可选 artifact technique。
