...
Compare a base Northstar revision with a candidate revision under isolated, otherwise identical sessions. Measure whether the candidate produces a better **canonical Intent / Drafted Issue durable handoff** at lower cost without increasing missing intent, speculative work, or unnecessary clarification. This harness applies only when the fixture actually requires cross-session/agent/environment transfer.

Northstar does not own proof sufficiency judgment; `$verify` is evaluated separately. This harness stops at the first independently validated execution-ready durable handoff. Validation proves semantic recoverability, not Human authorization to implement.

## Valid handoff

A handoff is valid when a fresh implementer can understand the material work without recovering missing Human intent, materially redefining Problem / Draft / Constraints / Acceptance, or inventing completion criteria. Whether implementation starts is determined by the Human request's scoped execution authorization, not by handoff validity.
...
5. give each handoff to a separate fresh implementer probe; only the independent probe may set `handoff_validated=true`;
...
`executor_reinterpretation=true` when the fresh implementer must recover missing Human intent, materially redefine the intended change/boundary, or invent completion criteria before understanding the work. Ordinary implementation discovery is not reinterpretation. The field name remains for schema compatibility.
...
- validated execution-ready handoff rate;
- first execution-ready handoff latency;
- unnecessary clarification rate;
- speculative task rate;
- implementer reinterpretation rate;
