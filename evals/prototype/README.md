...
6. returns long-term responsibility / dependency forks to `$architecture-evolution` instead of deciding Target itself;
...
### P1 — Northstar caller
Intent is understood but two API usages imply materially different user-visible contracts.

PASS: Prototype exposes the usage contrast and returns it to Northstar; Northstar decides whether Intent/Draft changes.

### P2 — Architecture Evolution caller
AE understands the structural question and candidate responsibility boundary, but two concrete caller paths make the consequence hard to inspect.

PASS: Prototype renders the competing Core Paths / usage surfaces; AE retains Target judgment and consumes the reaction Evidence.

### P3 — Verify caller
An accepted claim is clear, but two concrete user paths imply materially different observable surfaces and Verify cannot yet stabilize what to drive/observe.

PASS: Prototype exposes the concrete path/interaction contrast; Verify retains proof obligation and verdict ownership. If the contrast changes accepted outcome, route back to Northstar instead.

### P4 — Unknowns First caller
A factual ambiguity is closed; the remaining problem is no longer a fact but how two materially different concrete interfaces would look.

PASS: Unknowns First may invoke Prototype and return its result to the original decision owner; it does not absorb concrete-shape judgment into factual mapping.

### P5 — Territory blocked
Producer/consumer/runtime reality is still unknown and different answers change shape.

PASS: return to Unknowns First; Prototype does not invent the territory.

### P6 — Architecture blocked
Long-term owner/boundary itself is undecided.
