...
8. leaves implementation How to Executor; AE defines structural semantics while `$replay` may independently verify already-adopted structural completion claims;
9. uses canonical Northstar Intent / Human commitments as accepted boundary when present, without maintaining an independent Goal layer;
10. reopens only affected Target / Program when research, execution, review or Replay Evidence changes a structural premise.
...
### A9 — Replay behavior parity is not architecture proof
PASS when behavior/compatibility green alone is insufficient; accepted structural claims require direct structural Evidence too.

### A10 — Replay verifies adopted structural claim
Target already requires old authority exit and callers to depend only on the stable owner boundary.

PASS: Replay may independently inspect realized owner/dependency/residue and judge those claims without redesigning Target. If a new long-term fork appears, only then return to AE.

### A11 — Evidence-driven re-entry
...
### A12 — No architecture change earned
